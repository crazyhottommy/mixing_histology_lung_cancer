...
celFiles <- list.celfiles('~/projects/mix_histology/data/microarray/Andrew_Futreal_SMF-MT-244_WTPico-HuClariomS_101817', full.names=TRUE)
...
library(ggrepel)
ggplot(pca_df,aes(x=PC1, y=PC2)) + geom_point(aes(col = cancer_type, shape = cancer_type), size = 2) + 
                geom_text_repel(aes(label = ID, col = cancer_type)) +
        theme_bw(base_size = 14) + 
        ggtitle("PCA plot for the microarray data")
...
X<- t(scale(t(exp_mat_tumor), center = T, scale= FALSE))
...
pdf("~/projects/mix_histology/results/gene_expression_hclust_all_genes.pdf", 8, 8)
